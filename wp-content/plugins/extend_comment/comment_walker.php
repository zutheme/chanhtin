...
if ( ! class_exists( 'comment_walker' ) ) {  
    class comment_walker extends Walker_Comment {
        ...
        function start_el( &$output, $comment, $depth = 0, $args = array(), $id = 0 ) {
            ...
            $avatar_custom = get_comment_meta( $comment->comment_ID, 'avatar_comment', true );
			$user_id = get_comment( $comment_ID )->user_id;
			$avatar = esc_url(get_avatar_url($user_id));
            $rating = get_comment_meta( $comment->comment_ID, 'rating', true ); 
            if(!isset($avatar)){
				if(isset($avatar_custom)){
            ...
            <li <?php comment_class(empty( $args['has_children'] ) ? '' :'parent') ?> id="<?php echo $user_id; ?>-comment-<?php comment_ID() ?>">
            <?php if($depth==1) {?>
                      ...
                      <?php for( $i=0; $i < 5; $i++ ) { 
                        if($i < $rating) { ?>
                            <span class="item">
                                ...
                                <label class="lb_radio"><span class="fa fa-star-compact"></span></label>
                            </span>
                        <?php }else {?>
                         <span class="item">
                           <input type="radio" name="rating" class="rate" value="<?php echo $i; ?>" />
                           <label class="lb_radio"><span class="fa fa-star-empty"></span></label>
                         </span>
                        <?php }
                    } ?>
                      </span> 
