...
function additional_fields () {
  ...
  '<label for="title">' . __( 'avatar' ) . '</label>'.

  '<input id="title" name="avatar_comment" type="text" size="30"  tabindex="5" /></p>';
...
function extend_comment_meta_box ( $comment ) {
    ...
    $title = get_comment_meta( $comment->comment_ID, 'avatar_comment', true );
    ...
    <p>
        ...
        <label for="title"><?php _e( 'avatar Title' ); ?></label>

        <input type="text" name="avatar_comment" value="<?php echo esc_attr( $title ); ?>" class="widefat" />
...
function hatazu_comment_script() {
  if(is_single()) {
    wp_enqueue_style('comment-css', plugin_dir_url(__FILE__) . 'css/comment.css',array(), '0.8.9', false);
    wp_enqueue_script('comment-js', plugin_dir_url(__FILE__) .'js/comment.js', array(), '1.0.5', true );
...
function custom_avatar($avatar_defaults){
		$custom_avatar = plugin_dir_url(__FILE__) .'image/guest1.png';;
		$avatar_defaults[$custom_avatar] = "Default Avatar";
